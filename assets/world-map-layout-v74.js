/* v74 — region-map interaction/layout refinement.
   Keeps v73 calibrated pins, makes the map the only selector, moves controls into
   the requested order, highlights the selected pin, and gives Desert/Sewers maps.
*/
(()=>{
  'use strict';
  ...
  const SPECIAL_PINS={
    '卡利科沙漠':{
      ...
      spots:[
        {label:'池塘',match:['沙漠','池塘'],x:31,y:70}
      ]
    },
    '下水道':{
      places:[
  ...
  function addSpecialPins(img,region,card,mapBox){
    ...
    rows.forEach(({pin,target})=>{
      ...
      button.dataset.sdvRegion=region;
      button.dataset.sdvMode=mode;
      button.dataset.sdvPin=pin.label;
  ...
  function restructure(img){
    ...
    card.classList.add('sdv-world-map-card-v74');
    ...
    const targetGrid=[...card.children].find(el=>{
      if(el===mapBox||el.tagName!=='DIV')return false;
      const buttons=[...el.children].filter(x=>x.tagName==='BUTTON');
    ...
    if(targetGrid){
      ...
      targetGrid.dataset.sdvPinTargets='true';
    }
    ...
    const header=[...root.children].find(el=>{
      if(el===card||el===modeWrap||el===returnBtn||el.tagName!=='DIV')return false;
    ...
    addSpecialPins(img,region,card,mapBox);
  ...
  function refresh(){
    ...
    requestAnimationFrame(()=>{
      scheduled=false;
      document.querySelectorAll('img[alt$="區域地圖"]').forEach(restructure);
    });
  }

  const start=()=>{
    const observer=new MutationObserver(records=>{
      if(records.some(r=>r.addedNodes.length||r.removedNodes.length))refresh();
    });
    ...
    document.addEventListener('click',event=>{
      ...
      if(pin){
        const mapBox=pin.closest('.sdv-world-mapbox-v74');
        const img=mapBox?.querySelector('img[alt$="區域地圖"]');
        const region=regionFromAlt(img?.getAttribute('alt'));
        const mode=pin.classList.contains('sdv-world-pin-v73--spot')?'spots':'places';
        if(region)selected.set(`${region}:${mode}`,norm(pinLabel(pin)));
        setTimeout(refresh,0);
        return;
      }
      const text=norm(button.textContent);
      if(text===norm('📍 地點')||text===norm('🎣 釣點')){
        selected.clear();
        setTimeout(refresh,0);
      }
    },true);
