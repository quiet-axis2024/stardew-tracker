...
for token in ['./world-nav-data-v81.js?v=81','./world-nav-island-v81.js?v=81','./world-nav-v81.js?v=81','./sw-v81.js','deploy-v81']:
    if token not in idx: fail('v81 index missing '+token)
...
# Fishing must live on the map where it exists.
...
# The new runtime owns navigation with a stack and never moves legacy React nodes.
for token in ["stack:[D.root]","state.stack.push","state.stack.pop","data-kind=","地点／入口","钓点"]:
    if token not in ui: fail('v81 runtime missing '+token)
for forbidden in ['insertBefore(', 'replaceChild(', 'removeChild(']:
    if forbidden in ui: fail('v81 must not move/remove React-owned DOM: '+forbidden)

for name,text in [('Cloudflare',build),('Pages',pages)]:
    if 'python3 scripts/audit-world-v81.py' not in text: fail(name+' does not run v81 audit')
...
print('v81 route-graph World audit passed')
