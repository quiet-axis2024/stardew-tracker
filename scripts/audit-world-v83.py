...
for token in ['./world-fish-data-v83.js?v=83','./world-nav-v83.js?v=83','./sw-v83.js','deploy-v83','./app.js?v=83']:
    if token not in idx:
...
# Re-entering 查找 → 世界 must reset the standalone route instead of reviving stale React state.
...
if "stardew-tracker-v83" not in sw or 'world-fish-data-v83.js' not in sw or 'world-nav-v83.js' not in sw:
    fail('v83 service worker cache incomplete')
...
for name,text in [('Cloudflare',build),('Pages',pages)]:
    ...
    for f in ['world-fish-data-v83.js','world-nav-v83.js','sw-v83.js']:
        if f not in text:
