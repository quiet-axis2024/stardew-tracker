...
function decorateRegionTitle(){
  const root=document.getElementById('sdv-world-v83');if(!root)return;
  const title=root.querySelector('.sdv83-title');if(!title||title.querySelector('.sdv85-title-thumb'))return;
  const mapImg=root.querySelector('.sdv83-map>img');
  ...
  if(src){
    const img=document.createElement('img');img.className='sdv85-title-thumb';img.src=src;img.alt='';
    img.style.cssText='width:30px;height:22px;object-fit:cover;object-position:center;border:1px solid #c9a85f;border-radius:3px;image-rendering:pixelated;flex:0 0 auto;background:#e8efcf';
    title.prepend(img);
  }
}
function focusNpcCard(raw,attempt=0){
  ...
  if(target){target.scrollIntoView({block:'center',inline:'nearest',behavior:'auto'});return;}
