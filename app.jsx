...
function StardewTracker() {
  ...
  const [npcFindTimeV92,setNpcFindTimeV92]=useState(null);
    ...
    <Card style={{padding:10}}>
      <div style={{display:"grid",gridTemplateColumns:"104px minmax(0,1fr)",gap:11,alignItems:"start"}}>
        ...
        <div style={{minWidth:0}}>
          ...
          <div style={{fontSize:11.5,color:C.brown,marginTop:8,fontWeight:850}}>持有 {(data.profilePortrait||data.base.profileDataVerifiedV47)?`${Number(data.base.money||0).toLocaleString()}g`:"—"}</div>
          <div style={{fontSize:10.5,color:C.muted,marginTop:1}}>累計 {(data.profilePortrait||data.base.profileDataVerifiedV47)?`${Number(data.base.totalIncome||0).toLocaleString()}g`:"—"}</div>
  ...
  const renderWorldV87 = () => {
    ...
    return <div>
      ...
      {worldQuickV71==="npc"&&(()=>{const S=window.SDVNpcScheduleV91;if(!S?.resolve)return null;
        ...
        const effT=(npcFindViewV92==="place"&&npcFindTimeV92==null)?1300:npcFindTimeV92;
        ...
        return <Card style={{padding:8,background:"#EAF2FA",marginBottom:7}}>
          ...
          <div style={{display:"flex",gap:4,alignItems:"center",marginTop:6,flexWrap:"wrap"}}>{[["npc","按人"],["place","按地點"]].map(([v,l])=><button key={v} onClick={()=>setNpcFindViewV92(v)} style={pill(npcFindViewV92===v)}>{l}</button>)}<span style={{width:1,height:14,background:C.line}}/>{[[null,"整日"],[900,"9:00"],[1300,"13:00"],[1900,"19:00"],[2300,"23:00"]].map(([v,l])=><button key={l} onClick={()=>setNpcFindTimeV92(v)} style={pill(npcFindTimeV92===v)}>{l}</button>)}</div>
          {fest?<div style={{marginTop:7,padding:"7px 9px",borderRadius:8,background:"#FFF1CF",border:`1px solid ${C.line}`,fontSize:8.4,color:C.brown,fontWeight:900}}>🎪 {fest.text}：節日日以會場為準，平日行程不適用。</div>
